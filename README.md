...
The current pivot adds the assistant-first surface, removes job discovery from the product positioning, demotes the old opportunities page to a provided-input utility, and gives the assistant an immediate structured understanding and heuristic assessment for supplied jobs. Source HTML is now normalized into meaningful blocks before requirements/responsibilities are extracted, reducing the risk that ATS feeds collapse into unusable single-line text. The assistant now hands a supplied opportunity into application preparation and persists it to the authenticated application store when signed in, while retaining a local handoff for unauthenticated use. Career and Application Studio are framed around evidence, preparation and improvement rather than role discovery.

...
export function buildApplicationPack(job:JobRecord, profile:RovaProfile, resume:RovaResume|null):ApplicationPack {
  ...
  const answers=[
    ...
    {question:'What is your expected compensation?',answer:profile.targetSalary ? `My current target is around ₹${profile.targetSalary} LPA, with flexibility depending on the scope, location and total compensation of the role.` : 'I am open to discussing compensation based on the scope of the role and the overall package.'},
  ]

...
export async function POST(req: Request) {
  ...
  try {
    ...
    await admin.from('automation_events').insert({ user_id:job.user_id, automation_job_id:job.id, event_type:'claimed', state:'running', message:'Worker claimed the durable automation job.', metadata:{workerId} })
    return NextResponse.json({ job:{ id:job.id, jobId:job.id, company:job.company, role:job.role, applicationUrl:job.application_url, mode:job.mode, accountKey:`${job.user_id}`, candidate:{...profile,resumeUrl} } })

...
export default function Applications(){
 ...
 if(!job||!pack)return <main className="main standalone"><Link className="back" href="/">← Command</Link><div className="mono eyebrow">WAYO / APPLICATION STUDIO</div><h1 className="title">Prepare an application.</h1><p className="lead">Select a role from Job Intelligence first. WAYO builds a role-specific preparation pack from your stored profile and latest resume.</p><Link className="primary" href="/opportunities"><WandSparkles size={14}/> Find a role</Link></main>
 return <main className="main standalone"><Link className="back" href="/opportunities">← Job Intelligence</Link><div className="mono eyebrow">WAYO / APPLICATION STUDIO</div><div className="careerHead"><div><h1 className="title">{pack.targetRole}</h1><p className="lead">{job.company} · {job.location || 'Location unspecified'} · {job.workMode}</p></div><div className="targetBox"><FileText size={16}/><div><span>ATS readiness</span><strong>{pack.atsScore}/100</strong></div></div></div>
 ...
 <div className="footer">Application submission automation remains a controlled execution stage. WAYO prepares and reviews the application package before submission.</div></main>
}

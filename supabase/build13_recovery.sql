...
begin
  ...
  update public.automation_jobs j set state='running',lease_owner=p_worker_id,lease_expires_at=now()+make_interval(secs=>greatest(30,least(p_lease_seconds,900))),attempts=j.attempts+1,worker_task_id=gen_random_uuid()::text,updated_at=now()

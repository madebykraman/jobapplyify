...
const NAV = [
  { href: '/workspace', label: 'Command', icon: Home }, { href: '/opportunities', label: 'Opportunities', icon: BriefcaseBusiness },
  { href: '/applications', label: 'Applications', icon: FileText }, { href: '/resume', label: 'Resume', icon: FileText },
  { href: '/career', label: 'Career', icon: Target }, { href: '/automation', label: 'Automation', icon: Zap },
  { href: '/insights', label: 'Insights', icon: Gauge }, { href: '/growth', label: 'Growth', icon: Sparkles },
]
...
export function KindleapShell({ children }: { children: ReactNode }) {
  ...
  async function signOut(){ if(supabase) await supabase.auth.signOut(); router.push('/auth'); router.refresh() }
  return <div className="kl-shell">
    ...
    <div className="kl-main"><header className="kl-topbar"><button className="kl-menu" aria-label="Open navigation" onClick={() => setOpen(true)}><Menu size={19}/></button><Link href="/workspace" className="kl-mobile-brand">{BRAND.name}</Link><button className="kl-command" type="button" onClick={() => window.dispatchEvent(new Event('kindleap:command'))} aria-label="Open command search"><Search size={14}/><span>Search anything…</span><kbd>⌘ K</kbd></button><Link href="/profile" className="kl-avatar" aria-label="Profile">KA</Link></header><main className="kl-content">{children}</main><nav className="kl-mobile-nav" aria-label="Mobile navigation">{NAV.slice(0,4).map(({ href, label, icon: Icon }) => <Link key={href} href={href} className={active(href) ? 'active' : ''} aria-current={active(href) ? 'page' : undefined}><Icon size={17}/><span>{label}</span></Link>)}</nav></div>
  </div>
}
export function SectionHeader({ eyebrow, title, description, action }: { eyebrow?: string; title: ReactNode; description?: string; action?: ReactNode }) { return <div className="kl-section-head"><div>{eyebrow && <div className="kl-eyebrow">{eyebrow}</div>}<h1>{title}</h1>{description && <p>{description}</p>}</div>{action && <div className="kl-head-action">{action}</div>}</div> }
